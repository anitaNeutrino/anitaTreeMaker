...
void makeConcatanatedFiles(char *baseRunDir, char *outputDir)
{
  ...
  for(int run=12;run<=262;run++) {
    sprintf(gpsName,"%s/run%d/gpsEvent%d.root",baseRunDir,run,run);
  ...
  sprintf(gpsName,"%s/gpsEvent.root",outputDir);
...
//  TChain *turfInput = new TChain("turfRateTree");
//  char turfRateName[FILENAME_MAX];
//  for(int run=12;run<=262;run++) {
//    sprintf(turfRateName,"%s/run%d/turfRateFile%d.root",baseRunDir,run,run);
//    turfInput->Add(turfRateName);
//  }

//  TurfRate *theTurfRatePtr=0;
//  turfInput->SetBranchAddress("turf",&theTurfRatePtr);

//  sprintf(turfRateName,"%s/turfRateFile.root",outputDir);
//  TFile *fpTurfOut = new TFile(turfRateName,"RECREATE");
//  TTree *turfRateTree = new TTree("turfRateTree","Tree of ANITA Scalers and Thresholds");
//  turfRateTree->Branch("turf","TurfRate",&theTurfRatePtr);

//   Long64_t numTurfEntries=turfInput->GetEntries();
//   std::cout << "there are " << numTurfEntries << " entries\n";
//   starEvery=numTurfEntries/100;
//  if(starEvery==0) starEvery++;
//   for(Long64_t entry=0;entry<numTurfEntries;entry++) {
//    if(entry%starEvery==0)
//      std::cerr << "*";
//    turfInput->GetEntry(entry);
//    turfRateTree->Fill();    
//   }
//   std::cerr << "\n";
//   turfRateTree->AutoSave();
//   fpTurfOut->Close();
