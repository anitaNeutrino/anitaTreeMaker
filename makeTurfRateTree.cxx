...
TurfRateStructVer41_t theTurfRateVer41;
TurfRateStructVer40_t theTurfRateVer40;
TurfRateStructVer35_t theTurfRateVer35;
...
void makeTurfRateTree(char *inName, char *outName) {
   ...
   while(PosFile >> fileName) {
	 ...
	 if(gHdr.verId != VER_TURF_RATE) {
	    ...
	    switch(gHdr.verId) {
	    case 41:
	       if(gHdr.numBytes==sizeof(TurfRateStructVer41_t)) {
		...
		std::cout << "Size matches will proceed\n";
		  version=40;
	       ...
	       break;
	    case 35:
	       if(gHdr.numBytes==sizeof(TurfRateStructVer35_t)) {
		  std::cout << "Size matches will proceed\n";
		  version=35;
	       }
	       break;
	 ...
	 if(version==VER_TURF_RATE) {
	    ...
	    switch(version) {
	    case 41:
	       numBytesExpected=sizeof(TurfRateStructVer41_t);
	       numBytes=gzread(infile,&theTurfRateVer41,numBytesExpected);
	       break;
	    case 40:
	       ...
	       break;
	    case 34:
	       numBytesExpected=sizeof(TurfRateStructVer35_t);
	       numBytes=gzread(infile,&theTurfRateVer35,numBytesExpected);
	       break;
	    case 16:
...
void processTurfRate(int version) {
  static Int_t lastPpsNum=-1;
   ...
   if(version==VER_TURF_RATE) {
      ...
      switch(version) {	 
       case 41:
	  theTurfRateClass = new TurfRate(runNumber,theTurfRateVer41.unixTime,&theTurfRateVer41);
	  break;
      case 40:
	  theTurfRateClass = new TurfRate(runNumber,theTurfRateVer40.unixTime,&theTurfRateVer40);
	  break;
      case 35:
	  theTurfRateClass = new TurfRate(runNumber,theTurfRateVer35.unixTime,&theTurfRateVer35);
	  break;
       // case 16:
